...
- [x] **AGENTS.md** files inside individual collection subdirectories
      — 9 files written.
- [x] **Molecule** tests for every role — 50 smoke-test scenarios added
      (one per role across 9 leaf collections): 50 `molecule.yml` +
      50 `converge.yml` + 50 `verify.yml` + 50 `requirements.yml`
      files (180 YML + 50 dirs). Generated by
      `manage/gen_molecule_scenarios.py`. Each scenario uses
      `geerlingguy/docker-debian13-ansible:latest` as the test platform
      and runs the role with a synthetic `users_catalog` /
      `users_list` so the `mps.base.identity` resolution protocol
      always has something to resolve. The `requirements.yml` is
      auto-derived from each role's `meta/main.yml` dependencies and
      normalised to Galaxy collection names. Run via
      `just molecule <role>` from `manage/` (or `molecule test` from
      the role dir). Scenarios are smoke tests — they verify
      convergence, not specific artifact state. Per-role semantic
      verifications deferred to a follow-up.
